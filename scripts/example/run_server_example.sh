...
EXAMPLE_DIR="$ROOT_DIR/example/client_and_server"
...
# Find all Python files in the client_and_server directory, excluding utils.py
...
if [[ -n "$init_file" ]]; then
    ...
    if should_skip "$relative_path"; then
        echo "Skipping client_and_server/$relative_path..."
    else
        echo "Running client_and_server/$relative_path..."
        run_server_example "$init_file"
        if [ $? -ne 0 ]; then
            echo "Error running client_and_server/$relative_path"
            exit 1
        fi
        echo
    fi

...
EXAMPLE_DIR="$ROOT_DIR/example/client_and_server/crypto"
...
# Find all Python files in the crypto directory, excluding utils.py
...
for py_file in $python_files; do
    ...
    if should_skip "$relative_path"; then
        echo "Skipping crypto/$relative_path..."
        continue
    fi
    echo "Running crypto/$relative_path..."
    python "$py_file" --port "$PORT"
    if [ $? -ne 0 ]; then
        echo "Error running crypto/$relative_path"
        exit 1
    fi
    echo
done

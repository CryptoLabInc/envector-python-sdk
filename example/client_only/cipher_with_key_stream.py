...
def _run_example(eval_mode: str, key_id: str, preset: str):
    ...
    keygen = KeyGenerator(key_id=key_id, eval_mode=eval_mode, preset=preset)
    key_dict = keygen.generate_keys_stream()

    enc_key_stream = key_dict["enc_blob"]
    sec_key_stream = key_dict["sec_blob"]
    sec_key_stream_bytes = utils.get_key_stream(sec_key_stream)
    ...
    dec_single = cipher.decryptor.decrypt(single_ctxt.data[0], sec_key=sec_key_stream_bytes)
    ...
    dec_bulk = cipher.decryptor.decrypt(bulk_db_ctxt.data[0], sec_key=sec_key_stream_bytes)
    print(f"Get Decrypted Vector: first 10:\n  {dec_bulk[0:10]}")

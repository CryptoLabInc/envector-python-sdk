...
def main(args):
    ...
    query = [vectors[0]]
    # Use top_k=1 to avoid backend-side metadata lookup mismatches in some IVF configurations.
    #
    # NOTE: Some deployments may show eventual-consistency between searchability and metadata availability.
    # Retry a few times if GetMetadata isn't ready yet.
    result = None
    for _ in range(30):
        try:
            result = search_index.search(query, top_k=1, output_fields=["metadata"])[0]
            break
        except grpc.RpcError as exc:
            if exc.code() != grpc.StatusCode.UNAVAILABLE:
                raise
            details = exc.details() or ""
            if "GetMetadata" not in details and "No items found for the given index" not in details:
                raise
            time.sleep(1)
    if result is None:
        raise RuntimeError("GetMetadata did not become available after waiting.")
    print("\nTest PC Search")
    print(result)
